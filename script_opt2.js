...
const contactMap = {
  email: 'chunyu.jenny@qq.com',
  wechat: 'JennyJan_2020'
};

function setContactBadge(type) {
  if (!contactBadge) return;
  const value = contactMap[type];
  if (!value) return;
  contactBadge.textContent = value;
  contactBadge.hidden = false;
}

function clearContactActive() {
  contactTriggers.forEach(btn => btn.setAttribute('aria-expanded', 'false'));
}

if (contactBadge && contactTriggers.length) {
  contactTriggers.forEach(btn => {
    btn.addEventListener('click', event => {
      ...
      setContactBadge(btn.dataset.contact);
    });
  });
...
const heroNotebook = document.getElementById('heroNotebook');

if (heroNotebook) {
  heroNotebook.addEventListener('click', () => {
    const opened = heroNotebook.classList.toggle('is-open');
    heroNotebook.setAttribute('aria-expanded', opened ? 'true' : 'false');
  });
}
